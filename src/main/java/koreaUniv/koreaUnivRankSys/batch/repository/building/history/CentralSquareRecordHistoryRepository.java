...
public interface CentralSquareRecordHistoryRepository extends JpaRepository<CentralSquareRecordHistory, Long> {
    ...
    @Query("SELECT c FROM CentralSquareRecordHistory c WHERE c.member.id = :id ORDER BY c.ranking")
    List<CentralSquareRecordHistory> findTopByRanking(@Param("id")Long id, Pageable pageable);

    default CentralSquareRecordHistory findTop(Long id) {
