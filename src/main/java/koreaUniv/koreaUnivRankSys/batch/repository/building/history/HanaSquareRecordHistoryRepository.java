...
public interface HanaSquareRecordHistoryRepository extends JpaRepository<HanaSquareRecordHistory, Long> {
    ...
    @Query("SELECT h FROM HanaSquareRecordHistory h WHERE h.member.id = :id ORDER BY h.ranking")
    List<HanaSquareRecordHistory> findTopByRanking(@Param("id")Long id, Pageable pageable);

    default HanaSquareRecordHistory findTop(Long id) {
